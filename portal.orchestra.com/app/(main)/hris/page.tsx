        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            <span
              ...
              title={`${req.employee?.firstName} ${req.employee?.lastName}`}
            >
              {req.employee?.firstName} {req.employee?.lastName}
            </span>
            <span
              className="text-[10px] text-muted-foreground truncate max-w-[150px]"
              title={req.leaveType?.name}
            >
              {req.leaveType?.name}
            </span>
          </div>
  ...
  const stats = [
    {
      ...
      value: employees?.meta?.nextCursor ? '50+' : employees?.data?.length || 0,
      icon: Users,
      color: 'text-blue-600',
      bg: 'bg-blue-50',
    },
    ...
    <div className="p-6 space-y-8 max-w-7xl mx-auto">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-lg mt-1">
            Personnel management, attendance, and leave tracking summary.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((stat, i) => (
          <Card
            key={i}
            className="border-none shadow-md hover:shadow-xl transition-all group cursor-default h-32 overflow-hidden relative"
          >
            ...
            <CardContent className="p-6 flex items-center gap-5 h-full">
              <div
                className={`h-14 w-14 rounded-2xl ${stat.bg} ${stat.color} flex items-center justify-center shadow-inner`}
              >
                <stat.icon size={28} />
              </div>
              <div>
                <p className="text-3xl font-black tracking-tighter">{stat.value}</p>
                <p className="text-xs font-bold text-muted-foreground uppercase opacity-70 tracking-widest">
                  {stat.label}
                </p>
              </div>
            </CardContent>
            ...
            <CardHeader className="flex flex-row items-center justify-between border-b border-border/10 pb-4">
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  The latest applications from your team.
                </p>
              </div>
                ...
                <div className="space-y-4">
                  <div className="p-3 bg-white rounded-xl shadow-sm border border-indigo-100">
                    ...
                    <p className="text-xs text-muted-foreground mt-1">
                      Required attendance for all departments.
                    </p>
                  </div>
                </div>
              </CardContent>
