...
export const columns: Column<JobExecutionLog>[] = [
  ...
  },
  {
    ...
    cell: (item) => (
      <span className="text-green-600 font-medium">{item.processedCount}</span>
    ),
  },
  {
    header: "Errors",
    ...
    cell: (item) => (
      <span
        className={`font-medium ${item.errorCount > 0 ? "text-red-600" : "text-gray-600"}`}
      >
        {item.errorCount}
      </span>
    ),
  },
  {
    ...
    cell: (item) => (
      <span className="text-sm">
        {format(new Date(item.startedAt), "MMM dd, yyyy HH:mm")}
      </span>
    ),
  },
  {
