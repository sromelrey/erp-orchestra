...
export function getFormFields(context: FormFieldsContext): FormField[] {
  ...
  const fields: FormField[] = [
    // Status display (read-only) - ensure it's always included in formData
    {
      name: 'status',
      label: 'Status',
      type: 'select',
      ...
      width: 'half',
      options: [
        { value: GoodsReceiptStatus.DRAFT, label: 'Draft' },
        { value: GoodsReceiptStatus.CONFIRMED, label: 'Confirmed' },
        { value: GoodsReceiptStatus.CANCELLED, label: 'Cancelled' },
      ],
      defaultValue: GoodsReceiptStatus.DRAFT,
    },
    {
      name: 'receiptType',
      label: 'Receipt Type',
