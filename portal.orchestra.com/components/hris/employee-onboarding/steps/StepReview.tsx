...
export function StepReview({ data }: StepReviewProps) {
  ...
  const netSalary =
    (compensation.basicSalary || 0) + (compensation.allowances || 0) - totalDeductions;

  return (
    <div className="space-y-6">
      <div className="bg-green-50 p-4 rounded-lg">
        <p className="text-sm text-green-700">
          <strong>Review Complete:</strong> Please confirm all information before creating the
          employee record.
        </p>
      </div>
        ...
        <CardContent className="space-y-2">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <p className="text-sm">{new Date(jobDetails.hireDate).toLocaleDateString()}</p>
            </div>
            <div>
              <span className="text-sm font-medium text-gray-500">Employment Type:</span>
              <p className="text-sm">
                {jobDetails.employmentType?.replace('_', ' ') || 'Not specified'}
              </p>
            </div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
            <div>
              ...
              <p className="text-sm">
                {jobDetails.departmentId ? `ID: ${jobDetails.departmentId}` : 'Not assigned'}
              </p>
            </div>
            <div>
              <span className="text-sm font-medium text-gray-500">Designation:</span>
              <p className="text-sm">
                {jobDetails.designationId ? `ID: ${jobDetails.designationId}` : 'Not assigned'}
              </p>
            </div>
            <div>
              <span className="text-sm font-medium text-gray-500">Branch:</span>
              <p className="text-sm">
                {jobDetails.branchId ? `ID: ${jobDetails.branchId}` : 'Not assigned'}
              </p>
            </div>
          </div>
        </CardContent>
      ...
      <Card>
        ...
        <CardContent className="space-y-2">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <span className="text-sm font-medium text-gray-500">Allowances:</span>
              <p className="text-sm">
                ₱{(compensation.allowances || 0).toLocaleString()} (display only)
              </p>
            </div>
            <div>
              <span className="text-sm font-medium text-gray-500">Pay Frequency:</span>
              <p className="text-sm capitalize">
                {compensation.payFrequency?.replace('-', ' ') || 'Not specified'}
              </p>
            </div>
            <div>
            ...
            <div className="space-y-2">
              ...
              <div className="border-t pt-2 mt-4">
                <div className="flex justify-between font-medium">
                  ...
                  <span className="text-gray-900">₱{totalDeductions.toLocaleString()}</span>
                </div>
              </div>
            </div>
          )}
      ...
      <Card className="bg-blue-50 border-blue-200">
        ...
        <CardContent className="space-y-3">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              <span className="text-sm font-medium text-blue-700">Monthly Gross:</span>
              <p className="text-lg font-bold text-blue-900">
                ₱
                {(
                  (compensation.basicSalary || 0) + (compensation.allowances || 0)
                ).toLocaleString()}
              </p>
            </div>
            <div>
              <span className="text-sm font-medium text-blue-700">Monthly Deductions:</span>
              <p className="text-lg font-bold text-red-600">₱{totalDeductions.toLocaleString()}</p>
            </div>
            <div>
              <span className="text-sm font-medium text-blue-700">Monthly Net:</span>
              <p className="text-lg font-bold text-green-600">₱{netSalary.toLocaleString()}</p>
            </div>
          </div>
          <p className="text-sm text-blue-700 mt-4">
            <strong>What happens next:</strong> Clicking &rdquo;Confirm & Create Employee&rdquo;
            will create the employee record and associated compensation/deductions, then redirect to
            the employee&rdquo;s profile page.
          </p>
        </CardContent>
      </Card>
    </div>
