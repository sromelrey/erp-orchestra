...
export function usePermissionManager(role: any) {
  ...
  const modules = useMemo(() => {
    const moduleGroups = allPermissions.reduce((acc, permission) => {
    ...
    Object.keys(moduleGroups).forEach(module => {
      const moduleSelectedPermissions = selectedModule === module ?
        allPermissions.filter(p => p.module === module) : []
      moduleGroups[module] = {
        ...moduleGroups[module],
        selectedCount: moduleSelectedPermissions.length,
        totalCount: moduleGroups[module]?.length || 0
      }
    })
